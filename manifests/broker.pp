...
class openshift_origin::broker {
  ...
  if $::openshift_origin::conf_broker_usage_url == undef {
    cron { 'sync usage':
        ...
        command => "oo-admin-ctl-usage --list > /tmp/oo-usage && res=$(curl -sL -w \"%{http_code}\\n\" --data-urlencode 'file=`cat /tmp/oo-usage`' $::openshift_origin::conf_broker_usage_url -o /dev/null) && if [ $res -eq 200 ] then oo-admin-ctl-usage --sync fi",
        user => 'root', 
